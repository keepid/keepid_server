...
// this test is now deprecated, it does not work properly
public class AnnotationPDFServiceTest {
  private UserDao userDao = UserDaoFactory.create(DeploymentLevel.TEST);
  ...
  //
  //    File applicationPDF =
  //        new File(resourcesFolderPath + File.separator +
  // "Pennsylvania_Birth_Certificate_Form.pdf");
  //    String fileId = uploadFileAndGetFileId(applicationPDF, "BLANK_FORM");
  //
  //    JSONObject body = new JSONObject();
  //    body.put("applicationId", fileId);
  ...
  //    assertEquals(
  //        "An application for a birth certificate in the state of Pennsylvania. Requires a
  // driver's license photocopy to apply",
  ...
  //    try {
  //      File filled_out_pdf = new File(resourcesFolderPath + File.separator +
  // "ss-5_filled_out.pdf");
