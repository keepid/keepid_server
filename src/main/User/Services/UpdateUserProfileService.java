...
public class UpdateUserProfileService implements Service {
    ...
    if (updateRequest.has("email")) {
      ...
      if (email != null && !email.equals(JSONObject.NULL.toString())) {
        if (!ValidationUtils.isValidEmail(email)) {
          throw new ValidationException(UserValidationMessage.toUserMessageJSON(UserValidationMessage.INVALID_EMAIL));
        }
        user.setEmail(email);
      }
    }
    ...
    if (updateRequest.has("phone")) {
      ...
      if (phone != null && !phone.equals(JSONObject.NULL.toString())) {
        if (!ValidationUtils.isValidPhoneNumber(phone)) {
          throw new ValidationException(UserValidationMessage.toUserMessageJSON(UserValidationMessage.INVALID_PHONENUMBER));
        }
        user.setPhone(phone);
      }
    }
    ...
    if (updateRequest.has("address")) {
      ...
      if (address != null && !address.equals(JSONObject.NULL.toString())) {
        if (!ValidationUtils.isValidAddress(address)) {
          throw new ValidationException(UserValidationMessage.toUserMessageJSON(UserValidationMessage.INVALID_ADDRESS));
        }
    ...
    if (updateRequest.has("zipcode")) {
      ...
      if (zipcode != null && !zipcode.equals(JSONObject.NULL.toString())) {
        if (!ValidationUtils.isValidZipCode(zipcode)) {
          throw new ValidationException(UserValidationMessage.toUserMessageJSON(UserValidationMessage.INVALID_ZIPCODE));
        }
  ...
  private void updatePerson(JSONObject personJSON, OptionalInformation optionalInfo) throws ValidationException {
    ...
    // Merge fields from JSON into existing Person object
    if (personJSON.has("firstName")) {
      String firstName = getStringOrNull(personJSON, "firstName");
      if (firstName != null) {
        if (!ValidationUtils.isValidFirstName(firstName)) {
          throw new ValidationException(UserValidationMessage.toUserMessageJSON(UserValidationMessage.INVALID_FIRSTNAME));
        }
    ...
    if (personJSON.has("lastName")) {
      String lastName = getStringOrNull(personJSON, "lastName");
      if (lastName != null) {
        if (!ValidationUtils.isValidLastName(lastName)) {
          throw new ValidationException(UserValidationMessage.toUserMessageJSON(UserValidationMessage.INVALID_LASTNAME));
        }
    ...
    if (personJSON.has("birthDate")) {
      ...
      if (birthDateStr != null) {
        try {
          ...
          throw new ValidationException(UserValidationMessage.toUserMessageJSON(UserValidationMessage.INVALID_BIRTHDATE));
        }
      } else {
  ...
  private void updateBasicInfo(JSONObject basicInfoJSON, OptionalInformation optionalInfo) throws ValidationException {
  ...
  private <T> void updateFieldIfPresent(JSONObject json, String key, java.util.function.Consumer<T> setter, Class<T> type) {
    if (json.has(key)) {
      Object value = json.get(key);
      if (value == null || JSONObject.NULL.equals(value)) {
        setter.accept(null);
